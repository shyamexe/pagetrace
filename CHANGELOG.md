...
- A release now also publishes to GitHub Packages, under the scoped name
  `@shyamexe/pagetrace`. npmjs stays the registry to install from: GitHub
  Packages requires a `read:packages` token even for a public package, and does
  not support provenance. No code changed in this release.
